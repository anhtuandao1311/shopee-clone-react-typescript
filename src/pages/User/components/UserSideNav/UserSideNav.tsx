...
export default function UserSideNav() {
  ...
  // dung NavLink de active link de dang hon bang isActive
  return (
    <div>
      <div className='flex items-center gap-2 border-b border-b-gray-200 py-4'>
        <Link to={path.profile} className='h-10 w-10 flex-shrink-0 overflow-hidden rounded-full border border-black/10'>
          <img src={getAvatarUrl(profile?.avatar)} alt='' className='h-full w-full object-cover' />
        </Link>
        <div className='flex flex-col'>
          <div className='mb-1 font-semibold text-sm text-gray-600 break-all'>{profile?.email}</div>

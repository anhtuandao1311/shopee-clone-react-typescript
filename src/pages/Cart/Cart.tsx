...
export default function Cart() {
    ...
    setExtendedPurchase((prev) => {
      ...
      return (
        purchasesInCart?.map((purchase) => ({
          ...purchase,
          // extendedPurchaseObject[purchase._id]?.checked co dau ? vi luc dau prev la [] nen no se undefined
          checked:
            purchase._id === chosenPurchaseIdFromLocation || Boolean(extendedPurchaseObject[purchase._id]?.checked),
          disabled: false
        })) || []
      )
    })
  ...
  useEffect(() => {
    return () => {
      // de f5 thi state cua location.state se mat
  ...
  const totalCheckedPurchasesPrice = useMemo(
    () =>
      checkedPurchases.reduce((prev, curr) => {
        return prev + curr.price * curr.buy_count
      }, 0),
    [checkedPurchases]
  )
  ...
  const totalCheckedPurchasesSavingPrice = useMemo(
    () =>
      checkedPurchases.reduce((prev, curr) => {
        return prev + curr.price_before_discount * curr.buy_count
      }, 0) - totalCheckedPurchasesPrice,
    [checkedPurchases, totalCheckedPurchasesPrice]
  )

  const handleChecked = (purchaseId: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
    setExtendedPurchase(
      produce((draft) => {
  ...
  const handleQuantity = (purchaseId: number, value: number, enable: boolean) => {
    if (enable) {
      const purchase = extendedPurchase[purchaseId]
      setExtendedPurchase(
        produce((draft) => {
      ...
      <div className='container'>
        {extendedPurchase.length > 0 ? (
                                ...
                                onFocusOut={(value) =>
                                  handleQuantity(
                                    ...
                                    value >= 1 &&
                                      ...
                                      value !== (purchasesInCart as Purchase[])[index].buy_count // neu value khac voi buy_count thi moi update
                                  )
                                }
            ...
            <div className='sticky bottom-0 z-10 mt-8 flex flex-col rounded-sm border border-gray-100 bg-white p-5 shadow sm:flex-row sm:items-center'>
              ...
              <div className='mt-5 flex flex-col sm:ml-auto sm:mt-0 sm:flex-row sm:items-center'>
                ...
                <Button
                  ...
                  disabled={buyProductMutation.isPending || isFetching}
                >
                  Mua hàng
                </Button>
              </div>
          ...
          <div className='text-center'>
            <img src={cartPlaceholder} alt='ko co san pham' className='h-40 w-40 mx-auto' />
            <div className=' mt-5 font-bold text-gray-400'>Giỏ hàng của bạn còn trống</div>
            <div className='mt-8 text-center'>
              <Link to={path.home} className='bg-orange px-12 py-3 uppercase text-white hover:bg-orange/80 rounded-sm'>
                Mua Ngay
              </Link>
            </div>
          </div>
